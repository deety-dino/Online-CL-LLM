...
# 1. Tạo các file JSON Task Config
history_config = []
for one_data_name in dataset_list:
  pathlib.Path(f'./configs/{run_name}_configs/{one_data_name}').mkdir(
      parents=True, exist_ok=True
  )

  config = {'sampling strategy': 'full', 'dataset name': f'{one_data_name}'}
  history_config.append(deepcopy(config))

  dev_config = deepcopy(config_template)
  dev_config['Long_Sequence'].append(deepcopy(config))
  write_json(
      f'./configs/{run_name}_configs/{one_data_name}/dev_tasks.json', dev_config
  )

  train_config = deepcopy(config_template)
  train_config['Long_Sequence'].append(deepcopy(config))
  write_json(
      f'./configs/{run_name}_configs/{one_data_name}/train_tasks.json',
      train_config,
  )

  test_config = deepcopy(config_template)
  test_config['Long_Sequence'].extend(deepcopy(history_config))
  write_json(
      f'./configs/{run_name}_configs/{one_data_name}/test_tasks.json',
      test_config,
  )

# 2. Tạo Bash Script (.sh)
sh_str = rf"""#!/bin/bash
#SBATCH -J cl                           
#SBATCH -o cl-%j.out                       
#SBATCH -p compute 
...
deepspeed --num_gpus=2 src/run_llama_new.py \
   ...
   --logging_steps 10 \
   --metric_for_best_model eval_exact_match_for_{dataset_list[0]} \
...
# Các Task tiếp theo (Sequential Continual Learning)
...
for idx in range(len(dataset_list) - 1):
  curr_task = dataset_list[idx + 1]
  prev_task = dataset_list[idx]

  previous_lora_path_list.append(
      f'logs_and_outputs/{run_name}/outputs/{idx+1}-{prev_task}/saved_weights'
  )
  previous_lora_path = ','.join(previous_lora_path_list)

  if curr_task in max_steps_dict:
    gen_data_dir = 'generated_data/lora_gen_15datasets_t5_xl'
    steps_arg = f'--max_steps {max_steps_dict[curr_task]}'
  else:
    gen_data_dir = 'generated_data/lora_gen_long_llama'
    steps_arg = f'--num_train_epochs {num_train_epochs}'

  sh_str += rf"""
# Task {idx+2}: {curr_task}
deepspeed --num_gpus=2 src/run_llama_new.py \
   --do_train \
   --do_predict \
   --predict_with_generate \
   ...
   --task_config_dir configs/{run_name}_configs/{curr_task} \
   --output_dir logs_and_outputs/{run_name}/outputs/{idx+2}-{curr_task} \
   --per_device_train_batch_size 1 \
   --per_device_eval_batch_size 8 \
   --gradient_accumulation_steps 4 \
   --learning_rate {learning_rate} \
   ...
   --logging_strategy steps \
   --logging_steps 10 \
   --metric_for_best_model eval_exact_match_for_{curr_task} \
   --evaluation_strategy steps \
   --save_strategy steps \
   --save_total_limit 1 \
   --load_best_model_at_end \
...
rm -rf logs_and_outputs/{run_name}/outputs/{idx+2}-{curr_task}/checkpoint*
"""

with open(f'{run_name}.sh', 'w') as f:
  f.write(sh_str)

print(f'Generated {run_name}.sh successfully!')